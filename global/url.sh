...
function python_url_encode() {
  python3 -c "import sys, urllib.parse as u; print(u.quote(sys.argv[1], safe=''))" "$1"
}

function python_url_decode() {
  python3 -c "import sys, urllib.parse as u; print(u.unquote(sys.argv[1]))" "$1"
}
...
alias urldecode='python3 -c "import sys, urllib.parse as u; print(u.unquote(sys.stdin.read().strip()))"'
alias urlencode='python3 -c "import sys, urllib.parse as u; print(u.quote(sys.stdin.read().strip()))"'
...
'-e' | '--encode')
  input=${2:-$(</dev/stdin)}
  python_url_encode "$input"
  ;;
'-d' | '--decode')
  input=${2:-$(</dev/stdin)}
  python_url_decode "$input"
  ;;
'-h' | '--help')
  show_help
  ;;
